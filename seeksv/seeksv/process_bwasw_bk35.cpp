...
//void FindJunction(string file, int min_mapQ)
{
	...
	string chr, read_id, left_seq, right_seq, left_qual, right_qual,  chr1, read_id1, left_seq1, right_seq1, left_qual1, right_qual1;
	int pos, left_seq_len, right_seq_len, map_len_in_ref, pos1, left_seq_len1, right_seq_len1, map_len_in_ref1;
	char clipped_side, strand, clipped_side1, strand1;
	vector<pair<int, char> > cigar_vec, cigar_vec1;
	...
	while ((ret = samread(samfin, b) >= 0))
	{
		...
		Alignment up_alignment, down_alignment;
		pair<string, int> seq_num;

		if (read_id == read_id1)
		{
			int microhomology_length = -1;
			if (strand1 == strand && clipped_side1 != clipped_side || strand1 != strand && clipped_side1 == clipped_side)
			{
				Junction junction;
				SeqInfo up_seq_info, down_seq_info;
				if (strand1 == strand && clipped_side1 != clipped_side)
				{

					switch(clipped_side1)
					{
					case '5':
						up_alignment.set_value(chr, pos, left_seq, left_qual, right_seq, right_qual, cigar_vec, '3');
						down_alignment.set_value(chr1, pos1, left_seq1, left_qual1, right_seq1, right_qual1, cigar_vec1, '5');
						break;
					case '3':
						up_alignment.set_value(chr1, pos1, left_seq1, left_qual1, right_seq1, right_qual1, cigar_vec1, '3');
						down_alignment.set_value(chr, pos, left_seq, left_qual, right_seq, right_qual, cigar_vec, '5');
						break;
					}
					...
					if (up_alignment.left_seq.length() >= down_alignment.left_seq.length())
					{
						...
						up_seq_info.set_value(down_alignment.left_seq, cigar_vec, 0, 0, 0);
						down_seq_info.set_value(down_alignment.right_seq, down_alignment.cigar_vec, 0, 0, 1);
				...
				//	   << down_alignment.chr << '\t' << down_alignment.pos << '\t' << '+' << '\t' << microhomology_length << '\t' << read_id << endl;
					...
					else
					{
						...
						up_seq_info.set_value(down_alignment.left_seq, up_alignment.cigar_vec, 0, down_alignment.left_seq.length() - up_alignment.left_seq.length(), 0);
						down_seq_info.set_value(down_alignment.right_seq, down_alignment.cigar_vec, 0, 0, 1);
				...
				else if (strand1 != strand && clipped_side1 == clipped_side)
				{
					if (make_pair(chr1, pos1) < make_pair(chr, pos))
					{
						up_alignment.set_value(chr1, pos1, left_seq1, left_qual1, right_seq1, right_qual1, cigar_vec1, clipped_side);
						down_alignment.set_value(chr, pos, left_seq, left_qual, right_seq, right_qual, cigar_vec, clipped_side);
					}
					else
					{
						up_alignment.set_value(chr, pos, left_seq, left_qual, right_seq, right_qual, cigar_vec, clipped_side);
						down_alignment.set_value(chr1, pos1, left_seq1, left_qual1, right_seq1, right_qual1, cigar_vec1, clipped_side);
					}
					...
					if (clipped_side == '5')
					{
						if (up_alignment.right_seq.length() >= down_alignment.left_seq.length())
						{
							...
							up_seq_info.set_value(up_alignment.right_seq, up_alignment.cigar_vec, 0, 0, 0);
							down_seq_info.set_value(up_alignment.left_seq, cigar_vec, 0, 0, 1);
							//cerr << up_alignment.chr << '\t' << up_alignment.pos << '\t' << '-' << '\t'
						...
						else
						{
							microhomology_length = 0;
							junction.set_value(up_alignment.chr, up_alignment.pos, '-', down_alignment.chr, down_alignment.pos, '+');
							up_seq_info.set_value(down_alignment.left_seq, up_alignment.cigar_vec, 0, down_alignment.left_seq.length() - up_alignment.right_seq.length(), 0);
							down_seq_info.set_value(down_alignment.right_seq, down_alignment.cigar_vec, 0, 0, 1);
							//cerr << up_alignment.chr << '\t' << up_alignment.pos << '\t' << '-' << '\t'
						 	//<< down_alignment.chr << '\t' << down_alignment.pos << '\t' << '+' << '\t' << microhomology_length << '\t' << read_id << endl;
						}
					...
					else
					{
						if (up_alignment.left_seq.length() >= down_alignment.right_seq.length())
						{
							...
							cigar_vec = up_alignment.cigar_vec;
							MinusCigarRight(cigar_vec, microhomology_length);
							up_seq_info.set_value(down_alignment.right_seq, cigar_vec, 0, 0, 0);
							down_seq_info.set_value(down_alignment.left_seq, down_alignment.cigar_vec, 0, 0, 1);
							//cerr << up_alignment.chr << '\t' << up_alignment.pos - microhomology_length << '\t' << '+' << '\t'
						 //	<< down_alignment.chr << '\t' << down_alignment.pos << '\t' << '-' << '\t' << microhomology_length << '\t' << read_id << endl;
						}
						else
						{
							microhomology_length = 0;
							junction.set_value(up_alignment.chr, up_alignment.pos, '+', down_alignment.chr, down_alignment.pos, '-');
							up_seq_info.set_value(up_alignment.left_seq, up_alignment.cigar_vec, 0, 0, 0);
							down_seq_info.set_value(up_alignment.right_seq, down_alignment.cigar_vec, down_alignment.right_seq.length() - up_alignment.left_seq.length(), 0, 1);
		...
		read_id1 = read_id;
		strand1 = strand;
		cigar_vec1 = cigar_vec;
		clipped_side1 = clipped_side;
		left_seq_len1 = left_seq_len;
		right_seq_len1 = right_seq_len;
		left_seq1 = left_seq;
		right_seq1 = right_seq;
		left_qual1 = left_qual;
		right_qual1 = right_qual;
		chr1 = chr;
		pos1 = pos;
	}
}
