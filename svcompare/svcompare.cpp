...
public:
	...
	void set_value(int up_no, int down_no, string type, int sta) {up_support_read_no = up_no; down_support_read_no = down_no; sv_type = type; status = sta; }
};

void Usage(string prog, string command);
void ReadSVInfo(string file, map<Junction, Info> &junction2info);
void ReadCNVInfo(string file, map<Junction, Info> &junction2info);
...
void ReadSVInfo(string file, map<Junction, Info> &junction2info, map<pair<string, int>, int> &start2end)
{
	...
	while (fin >> type)
	{
		if (type == "inv" || type == "INV")
		{
			...
			map_it = start2end.lower_bound(make_pair(find_chr, start_pos));	
			
			if (map_it != start2end.end())
			{
				if (map_it->first.first == find_chr && end_pos > map_it->first.second)
					is_overlap = 1;
			}
			...
			{
				--map_it;
				if (map_it->first.first == find_chr && start_pos < map_it->first.second)
					is_overlap = 1;
			}
			
			if (is_overlap == 0)
			{
				Junction junction("chr17", start_pos - 1, '+', "chr17", end_pos, '-');
				junction2info.insert(make_pair(junction, info));
				junction.set_value("chr17", start_pos, '-', "chr17", end_pos + 1, '+');
				junction2info.insert(make_pair(junction, info));
			}
		}
		getline(fin, temp);
...
void ReadCNVInfo(string file, map<Junction, Info> &junction2info, map<pair<string, int>, int> &start2end)
{
	ifstream fin(file.c_str());
	if (!fin) 
	{
	...
	while (fin >> type)
	{
		if (type == "lins")
		{
			...
			while (getline(strm, al_pos, ';'))
			{
				...
				map_it = start2end.upper_bound(make_pair(find_chr, insert_pos));	
				if (map_it != start2end.begin())
				{
					--map_it;
					if (map_it->first.first == find_chr && map_it->first.second >= insert_pos)
						is_overlap = 1;
				}
				...
				if (is_overlap == 0)
				{
					Junction junction("chr17", insert_pos - 1, '+', "chr17", start_pos, '+');
					junction2info.insert(make_pair(junction, info_ins));
					junction.set_value("chr17", end_pos, '+', "chr17", insert_pos, '+');
					junction2info.insert(make_pair(junction, info_ins));
				}
			}
		}
		else if (type == "ldel")
		{
			...
			if (is_overlap == 0)
			{
				Junction junction("chr17", start_pos - 1, '+', "chr17", end_pos + 1, '+');
...
void CallSimu(int argc, char *argv[])
{
	...
	int type = SEEKSV;
	string narea_file;
	while ((c = getopt(argc-1, argv+1, "n:t")) >= 0)
	{
		switch (c)
		{
	...
	ReadSVInfo(file, simu_junction2info, start2end);
	file = argv[optind++];
	ReadCNVInfo(file, simu_junction2info, start2end);
	file = argv[optind++];
